...
**  `images` ディレクトリにある `lox-language-plugin-installed.png` を上書き保存する。

IMPORTANT: 後でプレビューを確認する。そのときに、下記のダミー画像が、自分の撮ったスナップに置き換わっていることを確認すること。

.Lox用の機能拡張をインストールした
image::lox-language-plugin-installed.png[{three-quarters-width}]

.実行した結果のスナップの例
image::lox-language-plugin-installed-exsample.png[{three-quarters-width}]
...
.Git Bashの動作を確認した
image::git-bash-installed.png[{three-quarters-width}]

.実行した結果のスナップの例
image::git-bash-installed-exsample.png[{three-quarters-width}]
...
.JDKのインストールを確認した
image::jdk-installed.png[{three-quarters-width}]

.実行した結果のスナップの例
image::jdk-installed-example.png[{three-quarters-width}]
...
'Shin - Chrome.lnk'*   astah_sysml.lic   hakoniwa-px4-win/ # <5>
----
<1> `cd` はディレクトリの移動コマンド。この例では、 `c` がドライブ名、後に続くのが自分のデスクトップのディレクトリ。 `kuboaki` はログイン名。
...
.AssignmentをCloneしたディレクトリを確認した
image::git-clone-result.png[{three-quarters-width}]

.実行した結果のスナップの例
image::git-clone-result-example.png[{three-quarters-width}]
...
.headコマンドでREADME.adocの中身を確認した
image::readme-adoc-console-result.png[{three-quarters-width}]

.実行した結果のスナップの例
image::readme-adoc-console-result-example.png[{three-quarters-width}]
...
.ChromeでREADME.adocをプレビューした
image::readme-adoc-chrome-result.png[{three-quarters-width}]

.実行した結果のスナップの例(1)
image::readme-adoc-chrome-result-example01.png[{three-quarters-width}]

.実行した結果のスナップの例(2)
image::readme-adoc-chrome-result-example02.png[{three-quarters-width}]
...
.VS CodeでREADME.adocをプレビューした
image::readme-adoc-vscode-result.png[{three-quarters-width}]

.実行した結果のスナップの例
image::readme-adoc-vscode-result-example.png[{three-quarters-width}]
...
NOTE: このファイル（ `README.adoc` ）が編集できたら、コミットしてプッシュしなさい。
...
.jLoxを動かした
image::jlox-orig-run-result.png[{three-quarters-width}]
...
.jLoxを使って別のディレクトリにあるプログラムを動かした
image::jlox-orig-aux-run-result.png[{three-quarters-width}]
...
image::breakfast-lox-code.png[{three-quarters-width}]
...
.`breakfast.lox` を実行した結果
image::breakfast.png[{three-quarters-width}]
...
[start=3]
. 次のLoxプログラム `doughnut.lox` を `codes` ディレクトリに作成しなさい。

.作成するLoxプログラム（ `doughnut.lox` ）
image::doughnut-lox-code.png[{three-quarters-width}]

[start=4]
. 下記手順を参考に、作成した `doughnut.lox` を実行しなさい。
...
> ^C # <2>
----
<1> `java` コマンドを使って、 jLox を起動した。実行プログラムとして `doughnut.lox` を指定した。
<2> `Ctrl + C` （CtrlキーとCキーを同時に押して）インタプリタを停止した。

ターミナルで、4章までのコードを使って `doughnut.lox` を実行している様子のスクリーンショットを撮りなさい。

.`doughnut.lox` を実行した結果
image::doughnut-lox-result.png[{three-quarters-width}]

NOTE: 4章が終わったら、下記手順を参考に、コミットしてプッシュしなさい。
...
ターミナルで、5章までのコードを使って 上記サンプルを実行している様子のスクリーンショットを撮りなさい。

.5章の実行結果
image::chap05-result.png[{three-quarters-width}]
...
.6章の実行結果
image::chap06-result.png[{three-quarters-width}]
...
.7章の実行結果
image::chap07-result.png[{three-quarters-width}]
...
.8章の実行結果
image::chap08-result.png[{three-quarters-width}]
...
.9章の実行結果
image::chap09-result.png[{three-quarters-width}]
...
ただし、Amazonで購入した場合とは、掲載する数や掲載までの時間に違いがあるのに注意すること。
--

. 書評の原稿を作成し、 `codes/amazon_review_draft.txt` として保存しなさい。
** 原稿にはレビューのポイントを表す表題をつけておく。
** 下記ファイルの内容は、ChromeのプレビューやGitHubのプレビューでは「 `Unresolved directive ...` 」となって、うまく表示されない。
** VS Codeのプレビューではファイルの内容が表示される。

.書評の原稿を作成して保存した（ `amazon_review_draft.txt` ）
[soure,console]
----
include::{sourcesdir}/amazon_review_draft.txt[]
----
...
.Amazonに掲載された書評
image::amazon-review-result.png[{three-quarters-width}]
...
IMPORTANT: このAssignmentについて、最終的な commit、pushの結果をもって再評価演習を提出したとみなす。
